...
kp_correct %>% group_by(fractal) %>% 
  summarize(mean_RT = mean(RT))
fit_brms_keyPress_RT <- brm(RT ~ 1 + fractal + (1|submission_id), data=kp_correct, seed = 617)
```
...
```{r}
h2_3 <- hypothesis(fit_brms_keyPress_RT, "fractalTRUE < 0")
h2_3
```
...
kp_correct %>% group_by(fractal) %>% 
  summarize(mean_RT = mean(RT))
fit_brms_keyPress_RT_complex <- brm(RT ~ 1 + fractal * nFOddball + (1|submission_id), data=kp_correct, seed = 617)
```


```{r}
h2_3_1 <- hypothesis(fit_brms_keyPress_RT_complex, "fractalTRUE < 0")
h2_3_1
```
...
```{r}
h2_3_2 <- hypothesis(fit_brms_keyPress_RT_complex, "nFOddballTRUE  < 0")
h2_3_2
```
...
```{r}
h2_3_3 <- hypothesis(fit_brms_keyPress_RT_complex, "fractalTRUE:nFOddballTRUE < 0")
h2_3_3
```
...
kp_correct %>% group_by(fractal) %>% 
  summarize(mean_RT = mean(RT))
fit_brms_keyPress_RT_complex_f_f <- brm(RT ~ 1 + fractal * FOddball + (1|submission_id), data=kp_correct, seed = 617)
```

```{r}
h2_3_4 <- hypothesis(fit_brms_keyPress_RT_complex_f_f, "fractalTRUE:FOddballTRUE < 0")
h2_3_4
```
